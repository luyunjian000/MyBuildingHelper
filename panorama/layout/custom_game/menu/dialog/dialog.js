class FiveDialog {
    ...
    init() {
        let _this = this
        let panel = this.el
        panel.BLoadLayout("file://{resources}/layout/custom_game/five_component/dialog/dialog.xml", false, false)
        ...
        this.panel = panel
        _this.setInputdefaultValue()
        GameUI.FiveDialog.push(this)
    }

    getinput() {
        return this.el.FindChildTraverse('FiveDialogInput').text
    }
    ...
    open() {
        GameUI.FiveDialog.forEach(element => {
            element.el.style.visibility = "collapse"
        });
